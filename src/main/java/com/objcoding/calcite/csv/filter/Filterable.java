...
public class Filterable {
    ...
    public static void main(String[] args) {
        try {
            // 字符串方式
            String model = Resources.asCharSource(Resources.getResource("model-filter.json"), Charset.defaultCharset()).read();
            Connection connection = DriverManager.getConnection("jdbc:calcite:model=inline:" + model);

            Statement statement = connection.createStatement();

            ResultSet resultSet = statement.executeQuery("select * from test_csv.TEST01 where name1 = 'hello'");
